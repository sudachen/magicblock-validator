...
impl Filter {
    pub fn new(
        ...
        Ok(Self {
            ...
            blocks_meta: FilterBlocksMeta::new(
                &config.blocks_meta,
                &limit.blocks_meta,
            )?,
            commitment,
    ...
    fn decode_pubkeys_into_vec(
        ...
        let mut vec = Self::decode_pubkeys(pubkeys, limit)
    ...
    pub fn get_filters<'a>(
        ...
        match message {
            ...
            Message::BlockMeta(message) => {
                self.blocks_meta.get_filters(message)
            }
        }
    }
            ...
            .filter_map(|(filters, message)| {
                if filters.is_empty() {
                    ...
                    Some(SubscribeUpdate {
                        filters,
                        update_oneof: Some(
                            message.to_proto(&self.accounts_data_slice),
                        ),
                    })
                }
            })
...
impl FilterAccounts {
    fn new(
        ...
        for (name, filter) in configs {
            ...
            this.filters.push((
                name.clone(),
                FilterAccountsData::new(&filter.filters)?,
            ));
        }
        Ok(this)
    }
            ...
            match &filter.filter {
                Some(AccountsFilterDataOneof::Memcmp(memcmp)) => {
                    let data = match &memcmp.data {
                        Some(AccountsFilterMemcmpOneof::Bytes(data)) => {
                            data.clone()
                        }
                        Some(AccountsFilterMemcmpOneof::Base58(data)) => {
                            anyhow::ensure!(
                                data.len() <= MAX_DATA_BASE58_SIZE,
                                "data too large"
                            );
                            bs58::decode(data).into_vec().map_err(|_| {
                                anyhow::anyhow!("invalid base58")
                            })?
                        }
                        Some(AccountsFilterMemcmpOneof::Base64(data)) => {
                            anyhow::ensure!(
                                data.len() <= MAX_DATA_BASE64_SIZE,
                                "data too large"
                            );
                            base64_engine.decode(data).map_err(|_| {
                                anyhow::anyhow!("invalid base64")
                            })?
                        }
                        None => {
                            anyhow::bail!("data for memcmp should be defined")
                        }
                    };
                    anyhow::ensure!(
...
impl<'a> FilterAccountsMatch<'a> {
    ...
    fn extend(
        ...
        map: &'a HashMap<Pubkey, HashSet<String>>,
        key: &Pubkey,
    ) {
    ...
    pub fn get_filters(&self) -> Vec<String> {
        self.filter
            ...
            .filter_map(|(name, filter)| {
                ...
                if af.account_required.contains(name)
                    && !self.account.contains(name)
                {
                    return None;
                }
                if af.owner_required.contains(name)
                    && !self.owner.contains(name)
                {
                    return None;
                }
    ...
    fn new(
        ...
        Ok(Self {
            filters: configs
                .iter()
                .map(|(name, filter)| {
                    (name.clone(), FilterSlotsInner::new(filter))
                })
                .collect(),
        })
    }
            ...
            self.filters
                ...
                .filter_map(|(name, inner)| {
                    if !inner.filter_by_commitment
                        || commitment == Some(message.status)
                    {
                        Some(name.clone())
                    } else {
                        None
            ...
            this.filters.insert(
                ...
                FilterTransactionsInner {
                    ...
                    signature: filter
                        ...
                        .as_ref()
                        .map(|signature_str| {
                            signature_str.parse().map_err(|error| {
                                anyhow::anyhow!("invalid signature: {error}")
                            })
                        })
                        .transpose()?,
            ...
            .filter_map(|(name, inner)| {
                ...
                if !inner.account_exclude.is_empty()
                    && message
                        ...
                        .iter()
                        .any(|pubkey| {
                            inner.account_exclude.binary_search(pubkey).is_ok()
                        })
                {
                    return None;
                }
                ...
                if !inner.account_required.is_empty() {
                    ...
                    let is_subset =
                        if inner.account_required.len() <= other.len() {
                            other.sort();
                            inner.account_required.iter().all(|pubkey| {
                                other.binary_search(&pubkey).is_ok()
                            })
                        } else {
                            false
                        };
...
impl FilterBlocks {
    fn new(
        ...
        for (name, filter) in configs {
            ...
            anyhow::ensure!(
                filter.include_transactions == Some(false)
                    || limit.include_transactions,
                "`include_transactions` is not allowed"
            );
            anyhow::ensure!(
                matches!(filter.include_accounts, None | Some(false))
                    || limit.include_accounts,
                "`include_accounts` is not allowed"
            );
            anyhow::ensure!(
                matches!(filter.include_entries, None | Some(false))
                    || limit.include_accounts,
        ...
        self.filters
            ...
            .map(|(filter, inner)| {
                ...
                (
                    vec![filter.clone()],
                    MessageRef::Block(
                        (message, transactions, accounts, entries).into(),
                    ),
                )
            })
            .collect()
...
mod tests {
    ...
    fn test_transaction_include_a() {
        ...
        let account_include =
            [account_key_a].iter().map(|k| k.to_string()).collect();
        transactions.insert(
            "serum".to_string(),
            SubscribeRequestFilterTransactions {
                vote: None,
    ...
    fn test_transaction_include_b() {
        ...
        let account_key_b = keypair_b.pubkey();
        let account_include =
            [account_key_b].iter().map(|k| k.to_string()).collect();
        transactions.insert(
            "serum".to_string(),
            SubscribeRequestFilterTransactions {
                vote: None,
    ...
    fn test_transaction_required_x_include_y_z_case001() {
        ...
        let account_required =
            [account_key_x].iter().map(|k| k.to_string()).collect();
        transactions.insert(
            "serum".to_string(),
            SubscribeRequestFilterTransactions {
                vote: None,
