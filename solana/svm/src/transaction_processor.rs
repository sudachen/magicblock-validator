...
use solana_program_runtime::{
    ...
    loaded_programs::{
        ForkGraph, LoadProgramMetrics, LoadedProgram,
        LoadedProgramMatchCriteria, LoadedProgramType, LoadedPrograms,
        LoadedProgramsForTxBatch, ProgramRuntimeEnvironment,
...
use solana_sdk::{
    ...
    instruction::{
        CompiledInstruction, InstructionError, TRANSACTION_LEVEL_STACK_HEIGHT,
    },
...
impl<FG: ForkGraph> TransactionBatchProcessor<FG> {
                    ...
                    (Ok(loaded_transaction), nonce) => {
                            ...
                            self.runtime_config.compute_budget
                        {
                            ...
                            let maybe_compute_budget =
                                ComputeBudget::try_from_instructions(
    ...
    /// blockhash or nonce.
    pub fn filter_executable_program_accounts<
        'a,
        CB: TransactionProcessingCallback,
    >(
        callbacks: &CB,
    ...
    fn replenish_program_cache<CB: TransactionProcessingCallback>(
        ...
        let mut missing_programs: Vec<(
            Pubkey,
            (LoadedProgramMatchCriteria, u64),
            ...
            let (program_to_load, task_cookie, task_waiter) = {
                ...
                if is_first_round {
                    loaded_programs_for_txs =
                        Some(LoadedProgramsForTxBatch::new(
                            self.slot,
                            loaded_programs_cache
                ...
                let task_waiter =
                    Arc::clone(&loaded_programs_cache.loading_task_waiter);
    ...
    fn execute_loaded_transaction<CB: TransactionProcessingCallback>(
        ...
        let transaction_accounts =
            std::mem::take(&mut loaded_transaction.accounts);
        ...
        fn transaction_accounts_lamports_sum(
            ...
            for i in 0..message.account_keys().len() {
                ...
                lamports_sum =
                    lamports_sum.checked_add(u128::from(account.lamports()))?;
            }
            Some(lamports_sum)
        }

        let lamports_before_tx = transaction_accounts_lamports_sum(
        ...
        let log_collector = if enable_log_recording {
            match log_messages_bytes_limit {
                ...
                Some(log_messages_bytes_limit) => {
                    Some(LogCollector::new_ref_with_limit(Some(
                        log_messages_bytes_limit,
                    )))
                }
            }
        ...
        let log_messages: Option<TransactionLogMessages> = log_collector
            .and_then(|log_collector| {
                Rc::try_unwrap(log_collector)
                    .map(|log_collector| {
                        log_collector.into_inner().into_messages()
                    })
                    .ok()
            });
        ...
        saturating_add_assign!(
            timings.details.changed_account_count,
            touched_account_count
        );

        let return_data =
            if enable_return_data_recording && !return_data.data.is_empty() {
                Some(return_data)
            } else {
                None
            };
    ...
    pub fn load_program<CB: TransactionProcessingCallback>(
        ...
        let environments =
            loaded_programs_cache.get_environments_for_epoch(effective_epoch);
        ...
        let mut loaded_program =
            match self.load_program_accounts(callbacks, pubkey, environments) {
                ...
                ProgramAccountLoadResult::InvalidAccountData(env) => {
                    Err((self.slot, env))
                }

                ProgramAccountLoadResult::ProgramOfLoaderV1orV2(
                    program_account,
                ...
                ProgramAccountLoadResult::ProgramOfLoaderV3(
                    ...
                    .ok_or(
                        Box::new(InstructionError::InvalidAccountData).into(),
                    )
                    .and_then(|programdata| {
                        Self::load_program_from_bytes(
                            ...
                            programdata,
                            program_account.owner(),
                            program_account.data().len().saturating_add(
        ...
        if let Some(recompile) = recompile {
            loaded_program.effective_slot = loaded_program.effective_slot.max(
                self.epoch_schedule.get_first_slot_in_epoch(effective_epoch),
            );
            loaded_program.tx_usage_counter = AtomicU64::new(
                recompile.tx_usage_counter.load(Ordering::Relaxed),
            );
            loaded_program.ix_usage_counter = AtomicU64::new(
                recompile.ix_usage_counter.load(Ordering::Relaxed),
            );
        }
    ...
    fn load_program_accounts<CB: TransactionProcessingCallback>(
        ...
        if loader_v4::check_id(program_account.owner()) {
            return solana_loader_v4_program::get_state(program_account.data())
                .ok()
                .and_then(|state| {
                    (!matches!(state.status, LoaderV4Status::Retracted))
                        .then_some(state.slot)
                })
                .map(|slot| {
                    ProgramAccountLoadResult::ProgramOfLoaderV4(
                        program_account,
        ...
        if !bpf_loader_upgradeable::check_id(program_account.owner()) {
            return ProgramAccountLoadResult::ProgramOfLoaderV1orV2(
                program_account,
            );
        }
        ...
        }) = program_account.state()
        {
            let programdata_account =
                match callbacks.get_account_shared_data(&programdata_address) {
                    None => return ProgramAccountLoadResult::AccountNotFound,
                    Some(account) => account,
                };
    ...
    fn inner_instructions_list_from_instruction_trace(
        ...
        for index_in_trace in
            0..transaction_context.get_instruction_trace_length()
        {
            if let Ok(instruction_context) = transaction_context
                .get_instruction_context_at_index_in_trace(index_in_trace)
            {
                ...
                } else if let Some(inner_instructions) =
                    outer_instructions.last_mut()
                {
                    let stack_height =
                        u8::try_from(stack_height).unwrap_or(u8::MAX);
...
mod tests {
    ...
    fn test_inner_instructions_list_from_instruction_trace() {
        ...
        for (index_in_trace, stack_height) in
            instruction_trace.into_iter().enumerate()
        {
            while stack_height
