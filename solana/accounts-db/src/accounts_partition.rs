...
use std::{collections::HashSet, mem, ops::RangeInclusive};
...
pub fn get_partition_from_slot_indexes(
    ...
    let mut start_partition_index =
        partition_index_from_slot_index(start_slot_index, cycle_params);
    let mut end_partition_index =
        partition_index_from_slot_index(end_slot_index, cycle_params);
...
pub fn variable_cycle_partition_from_previous_slot(
    ...
    let (current_epoch, current_slot_index) =
        epoch_schedule.get_epoch_and_slot_index(slot);
...
pub fn pubkey_range_from_partition(
    ...
    start_pubkey[0..PREFIX_SIZE]
        .copy_from_slice(&start_key_prefix.to_be_bytes());
    ...
    if start_index != end_index {
        ...
        if start_index != 0 {
            start_pubkey[0..PREFIX_SIZE].copy_from_slice(
                &start_key_prefix.saturating_sub(1).to_be_bytes(),
            );
        ...
        if end_index != partition_count - 1 && end_index != 0 {
            end_pubkey[0..PREFIX_SIZE].copy_from_slice(
                &end_key_prefix.saturating_add(1).to_be_bytes(),
            );
...
impl RentPayingAccountsByPartition {
    ...
    pub fn add_account(&mut self, pubkey: &Pubkey) {
        let partition_end_index =
            partition_from_pubkey(pubkey, self.partition_count);
...
pub(crate) mod tests {
    ...
    fn test_rent_eager_pubkey_range_maximum() {
        ...
        assert_eq!(
            range,
            Pubkey::new_from_array([
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ONE, 0x00, 0x00,
        ...
        assert_eq!(
            ...
            Pubkey::new_from_array([
                ...
                ..=Pubkey::new_from_array([
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, FD, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff
                ])
    ...
    fn test_rent_eager_pubkey_range_noop_range() {
        ...
        assert_eq!(
            ...
            Pubkey::new_from_array([0x00; 32])
                ..=Pubkey::new_from_array([
                    0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x54, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff
                ])
        ...
        let same = Pubkey::new_from_array([
            0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0x00, 0x00, 0x00,
    ...
    #[test]
    #[should_panic(
        expected = "range start is greater than range end in BTreeMap"
    )]
    ...
    fn pubkey_max_value() -> Pubkey {
        let highest =
            Pubkey::from_str("JEKNVnkbo3jma5nREBBJCDoXFVeKkD56V3xKrvRmWxFG")
                .unwrap();
    ...
    fn test_rent_eager_pubkey_range_not_dividable() {
        ...
        assert_eq!(
            ...
            Pubkey::new_from_array([
                ...
                ..=Pubkey::new_from_array([
                    0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xa9, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff
                ])
    ...
    fn test_rent_eager_pubkey_range_gap() {
        ...
        assert_eq!(
            range,
            Pubkey::new_from_array([
                0x02, 0x82, 0x5a, 0x89, 0xd1, 0xac, 0x58, 0x9c, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00
            ])
                ..=Pubkey::new_from_array([
                    0x15, 0x3c, 0x1d, 0xf1, 0xc6, 0x39, 0xef, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                    0xff, 0xff
                ])
    ...
    #[test]
    fn test_add() {
        let mut test = RentPayingAccountsByPartition::new(
            &EpochSchedule::custom(32, 0, false),
        );
