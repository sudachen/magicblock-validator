...
impl ActiveStats {
    ...
    fn update_and_log(
        ...
        match item {
            ActiveStatItem::Clean => {
                datapoint_info!("accounts_db_active", ("clean", value, i64))
            }
            ActiveStatItem::SquashAncient => {
                datapoint_info!(
                    "accounts_db_active",
                    ("squash_ancient", value, i64)
                )
            }
            ...
            ActiveStatItem::Flush => {
                datapoint_info!("accounts_db_active", ("flush", value, i64))
            }
            ActiveStatItem::HashDeDup => {
                datapoint_info!(
                    "accounts_db_active",
                    ("hash_dedup", value, i64)
                )
            }
            ActiveStatItem::HashMerkleTree => {
                datapoint_info!(
                    "accounts_db_active",
                    ("hash_merkle_tree", value, i64)
                )
            }
